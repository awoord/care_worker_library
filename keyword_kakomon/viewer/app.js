...
function renderQuestions(data) {
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    if (q.explanation && String(q.explanation).trim()) {
      var explText = String(q.explanation).trim();
      if (/^https?:\/\//i.test(explText)) {
        explBody.textContent = "";
        var link = document.createElement("a");
        link.href = explText;
        link.target = "_blank";
        link.rel = "noopener noreferrer";
        link.textContent = "過去問ドットコムで解説を見る";
        explBody.appendChild(link);
      } else {
        fillWithRuby(explBody, explText);
      }
    } else {
