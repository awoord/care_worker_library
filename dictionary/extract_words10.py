...
if __name__ == "__main__":
    ...
    try:
        ...
        # 1. ひらがな1文字または2文字の単語を取り出し、(原形, 表記, 品詞) の組みを作成する
        target_words = [
            (item["base_form"], item["surface"], item["pos"])
            for item in terms
            if re.fullmatch(r"[ぁ-ん]{1,2}", item["surface"])
        ]

        # 2. 同じ組みが何回出てきたかを自動で集計する
        counts = Counter(target_words)

        print("【抽出された単語一覧（ひらがな1〜2文字 / 出現回数順）】")
        # 3. 集計結果を「出現回数が多い順」に取り出して表示する
        for (base_form, surface, pos), count in counts.most_common():
            print(
