// ==========================================================
// 単語シート生成・ステージ進行（1問ずつ生成演出 ＋ 25%画面フィット版）
// ==========================================================
...
function createFlashCardsByCategories() {
  ...
  // ------------------------------------------
  // 3. 単一単語シートの文字・数式上書き関数（1問ずつ生成演出 ＋ 25%サイズ調整）
  // ------------------------------------------
  function updateSingleCategoryContent(sheet, chunkData, category, stageNum) {
    ...
    c2Cell.setFontColor(themeColor).setBackground(themeBg).setFontSize(48);
  
    var d2Cell = sheet.getRange("D2");
    d2Cell.clearDataValidations().clearContent().setFontSize(36).setFontColor("#E65100").setBackground(themeBg);
    ...
    for (var i = 0; i < 5; i++) {
      ...
      if (i < chunkData.length) {
        ...
        bCell.setValue(false).setFontColor("#1A73E8").setFontSize(48);
        ...
        cCell.setFormula(titleFormula).setFontSize(40);
  
        var detailFormula = '=IF(B' + row + '=TRUE, "' + escapeFormulaText(meaning) + '" & CHAR(10) & "' + escapeFormulaText(example) + '", "")';
        detailCell.setFormula(detailFormula).setFontSize(36);
  ...
  // ------------------------------------------
  // 4. 「覚」シートの特定カテゴリーのみ高速上書き関数（25%サイズ調整）
  // ------------------------------------------
  function updateStudySheetCategory(categoryName, chunkData) {
    ...
    for (var w = 0; w < 5; w++) {
      ...
      if (w < chunk.length) {
        ...
        bCell.setValue("\n▪").setFontSize(36);
        cCell.setValue(cardText).setFontSize(36);
      } else {
        bCell.setValue("");
        cCell.setValue("");
      }
    ...
    for (var h = 0; h < 5; h++) {
      ...
      sheet.setRowHeight(targetRow, Math.max(280, autoHeight - 60));
    }
  }
  
  // ------------------------------------------
  // 5. 初回描画用の完全設定関数（25%サイズ調整）
  // ------------------------------------------
  function applyStageDataToSheet(sheet, data, savedCheckStates, category, stageNum, wordCount, themeColor, themeBg) {
    ...
    // 列の幅を画面にぴったり収まる1260pxに設定
    sheet.setColumnWidth(1, 30);   // A列: 左余白
    sheet.setColumnWidth(2, 110);  // B列: チェックボックス用
    sheet.setColumnWidth(3, 910);  // C列: 単語テキスト本体
    sheet.setColumnWidth(4, 180);  // D列: D2クリアボックス用
    sheet.setColumnWidth(5, 30);   // E列: 右余白
  
    // 行の高さ設定
    sheet.setRowHeight(1, 48);
    sheet.setRowHeight(2, 120);    // 2行目の高さを120pxに広げてゆとりを確保
    ...
    c2Cell.setHorizontalAlignment("left").setVerticalAlignment("middle")
          ...
          .setFontSize(48).setFontWeight("bold").setFontFamily("M PLUS 1p");
    ...
    if (wordCount > 0 && checkedCount >= wordCount) {
      d2Cell.clearDataValidations().clearContent().insertCheckboxes()
            ...
            .setFontSize(48).setFontColor("#D97706").setBackground(themeBg);
    } else {
      ...
      if (wordCount === 0) {
        ...
        d2Cell.setFormula('="⭐️" & ' + totalLearnedCalc)
              .setHorizontalAlignment("center").setVerticalAlignment("middle")
              .setFontColor("#E65100").setFontSize(36).setFontWeight("bold").setFontFamily("M PLUS 1p");
      }
    }
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      sheet.setRowHeight(currentRow + 2, 48);
      ...
      bCell.setValue(isChecked).setBackground("#FFFFFF").setFontColor("#1A73E8")
           .setFontSize(48).setHorizontalAlignment("center").setVerticalAlignment("middle");
  
      var titleBase = number + ". " + word + "（" + ruby + "）";
      var titleWithEn = titleBase + "  " + english;
  
      var cCell = sheet.getRange("C" + currentRow);
      cCell.setValue(titleWithEn).setBackground("#FFFFFF").setFontSize(40)
      ...
      detailCell.setValue(meaning + "\n" + example).setBackground("#FFFFFF").setFontSize(36)
                .setFontColor("#333333").setFontWeight("normal").setVerticalAlignment("middle")
    ...
    if (actualRows > 0) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        sheet.setRowHeight(currentRow, Math.max(120, headerH));
  
        sheet.autoResizeRows(currentRow + 1, 1);
        var autoH = sheet.getRowHeight(currentRow + 1);
        sheet.setRowHeight(currentRow + 1, autoH + 40);
