...
var AUTH_STORAGE_KEY = "kaigo_words_google_credential_v1";
...
function isKaigoPublicDeploy() {
  return true;
}

function getStorageKey(base) {
  if (authState.loggedIn && authState.userId) {
    return base + "_kaigo_" + authState.userId;
  }
  return base + "_kaigo_guest";
}

function getApiUrl(extraParams) {
  var parts = ["app=kaigo", "_t=" + Date.now()];
  if (extraParams) {
...
function persistAuthCredential(credential, profile) {
  try {
    if (!credential) {
    ...
    sessionStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify({
      idToken: credential,
      userId: (profile && profile.userId) || authState.userId || "",
      email: (profile && profile.email) || authState.email || "",
      savedAt: Date.now()
    }));
  } catch (err) {}
}
...
var guestFlashCursorsMemory = {};

function clearGuestPersistedProgress() {
  try {
    localStorage.removeItem("saved_pending_checks_kaigo_guest");
    localStorage.removeItem("saved_today_committed_learned_kaigo_guest");
    localStorage.removeItem("saved_today_skipped_words_kaigo_guest");
    localStorage.removeItem("saved_achieved_dates_kaigo_guest");
    localStorage.removeItem("care_worker_words_cache_v3_kaigo_guest");
    localStorage.removeItem("flash_category_cursors_v1_kaigo_guest");
    localStorage.removeItem("saved_tracked_jst_date_key_kaigo_guest");
    sessionStorage.removeItem("care_worker_flash_session_v2_kaigo_guest");
    sessionStorage.removeItem("saved_local_learned_overrides_kaigo_guest");
  } catch (err) {}
...
function updateAuthBarUI() {
  ...
  var loginMount = document.getElementById("googleLoginMount");
  var logoutBtn = document.getElementById("googleLogoutBtn");
  var guestNote = document.getElementById("authGuestNote");
  if (!statusEl) return;

  if (isLoggedIn()) {
    statusEl.textContent = authState.email
      ? ("ログイン中: " + authState.email)
      : "ログイン中";
    if (loginBtn) loginBtn.hidden = true;
    if (loginMount) loginMount.hidden = true;
...
function setAuthFromToken(idToken, profile) {
  ...
  authState.loggedIn = !!idToken;
  if (profile && profile.userId) {
    authState.userId = profile.userId;
  } else if (!idToken) {
    authState.userId = "";
  }
...
function handleGoogleCredentialResponse(response) {
  ...
  setAuthFromToken(response.credential, {});
...
function initGoogleAuth() {
  updateAuthBarUI();
  var saved = loadAuthCredential();
  if (saved && saved.idToken) {
    setAuthFromToken(saved.idToken, {
      userId: saved.userId || "",
      email: saved.email || ""
    });
  }
  ...
  if (!GOOGLE_CLIENT_ID) {
    ...
    var statusEl = document.getElementById("authStatusText");
    if (statusEl && !isLoggedIn()) {
      statusEl.textContent = "ゲスト（ログイン未設定）";
    }
    return;
  }
  ...
  function setupGis() {
    ...
    var mount = document.getElementById("googleLoginMount");
    if (mount) {
      mount.hidden = false;
      mount.innerHTML = "";
...
function loadUiState() {
  ...
  if (!isLoggedIn()) {
    ...
    // ゲストはリロードで最初から（しおり・セッションを復元しない）
    flashSession.cat = "";
    flashSession.wordNames = [];
...
function loadFlashCategoryCursors() {
  if (!isLoggedIn()) {
    return guestFlashCursorsMemory || {};
  }
  try {
...
function saveFlashCategoryCursors(cursors) {
  if (!isLoggedIn()) {
    guestFlashCursorsMemory = cursors || {};
    return;
  }
...
function flushPendingChecksOnce() {
  ...
  return postKaigoJson(postPayload).then(function (body) {
    ...
    // POST がリダイレクトで GET 化すると success が付かない
    if (!body || body.success !== true) {
...
(function restoreAuthBeforePrefetch() {
  var saved = loadAuthCredential();
  if (saved && saved.idToken) {
